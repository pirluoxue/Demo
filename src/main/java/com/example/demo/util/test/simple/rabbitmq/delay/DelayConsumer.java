...
public class DelayConsumer {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        //声明死信队列Exchange和Queue
        channel.exchangeDeclare(DEAD_QUEUE_EXCHANGE, EXCHANGE_TYPE);
        channel.queueDeclare(DEAD_QUEUE_NAME, true, false, false, null);
        channel.queueBind(DEAD_QUEUE_NAME, DEAD_QUEUE_EXCHANGE, "#");

        //声明普通Exchange
        channel.exchangeDeclare(QUEUE_EXCHANGE, EXCHANGE_TYPE, true, false, null);

        //声明消息队列, 指定死信队列为DEAD_QUEUE_EXCHANGE
        Map<String, Object> arguments = new HashMap<>();
        // x-dead-leeter-exchange属性用于指定死信队列为DEAD_QUEUE_EXCHANGE
        arguments.put("x-dead-letter-exchange", DEAD_QUEUE_EXCHANGE);
        //x-message-ttl属性用于配置队列最大存在时间
        arguments.put("x-message-ttl", 60000);

        channel.queueDeclare(QUEUE_NAME, true, false, false, arguments);

        // 绑定队列和Exchange
        channel.queueBind(QUEUE_NAME, QUEUE_EXCHANGE, ROUTE_KEY);
...
//        consumerChannel.queueDeclare(QUEUE_NAME, true, false, false, arguments);
        ...
        Consumer consumer = new DefaultConsumer(channel) {
            ...
            public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                ...
                System.out.println("C [x] Received '" + message + "'     LocalDateTime: " + LocalDateTime.now());
            }
        };
